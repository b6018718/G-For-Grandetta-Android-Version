...
bool Screen::init(SDL_Window *& gWindow, SDL_Surface *& gScreenSurface)
{
    ...
    {
        SDL_Log("SDL Initalisation failed. Error Message: %s\n", SDL_GetError());
        success = false;
    }
    ...
    {
        //Create Window
        gWindow = SDL_CreateWindow("G For Grandetta Enhanced Edition", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
        if (gWindow == NULL)
        {
...
void Screen::displayText(string text, float x, float y, TTF_Font* newfont) //Centered Middle
{
    ...
    int fontSize = TTF_FontHeight(newfont) - 1;
    ...
    SDL_Rect textLocation = { static_cast<int>((float)(x - (gText->w)/ 2.0) / 960.0 * SCREEN_WIDTH), static_cast<int>((float)(y + (6.0 / 640.0 * SCREEN_HEIGHT))),
                              static_cast<int>(((fontSize * (length + 1.0))/960.0) * SCREEN_WIDTH), static_cast<int>(fontSize / 640.0 * SCREEN_HEIGHT)};
...
void Screen::displayLeftText(string text, float x, float y, TTF_Font* newfont)	//Centered Left
{
    ...
    gTemp = TTF_RenderText_Solid(newfont, charText, foregroundColor);
    gText = SDL_ConvertSurfaceFormat(gTemp, SDL_GetWindowPixelFormat(gWindow), 0);
    SDL_Rect textLocation = { static_cast<int>((x/960.0) * SCREEN_WIDTH), static_cast<int>(((y - 24.0)/640.0) * SCREEN_HEIGHT),
                              static_cast<int>(((fontSize * (length + 1.0))/960.0) * SCREEN_WIDTH), static_cast<int>(fontSize / 640.0 * SCREEN_HEIGHT) };
...
bool Screen::messageBox(string line1, string line2, TTF_Font* font)	//35 Character Limit Per Line
{
    ...
    loadMedia(gMessage, "images/messageBox.png");
    SDL_Rect screenDest;
    screenDest.x = 0;
    screenDest.y = 0;
    screenDest.h = SCREEN_HEIGHT;
    screenDest.w = SCREEN_WIDTH;
    SDL_BlitScaled(gMessage, NULL, gScreenSurface, &screenDest);
...
{
    float canXC = (float)(960 - player.spriteSizeX) / 2;	//Can X Camera, If there is room left for the map to move
    float canYC = (float)(640 - player.spriteSizeY) / 2;	//Can Y Camera
    ...
    {
        bg.x = 960 - mapZone.xDim;	//Is as far right as possible
    ...
    {
        bg.y = 640 - mapZone.yDim + 160; //Is as far down as possible
    }
    else
    {
        ...
        switch (player.currentMap)
        {
            case 0:	//Village Events
                if ((player.currentQuest == 0 && player.currentQuestPoint < 2) || player.currentQuest == 7)
                {
                    ...
                    //		maps.removeItem(0, "villageDog");
                    //	loadMapMedia(gPlaySurface, "images/bg0.png");
                    //	player.questLoaded = true;
                    //}
            ...
            case 3:	//Castle Events
                ...
                else if (player.currentQuest == 6)
                {
                    ...
                    //loadMapMedia(gPlaySurface, "images/bg3.png");
                }
    ...
    dt.w = SCREEN_WIDTH;
    dt.h = SCREEN_HEIGHT;
    //loadMedia(gPlaySurface, "images/bg0.png");
    SDL_BlitScaled(gPlaySurface, &bg, gScreenSurface, &dt);

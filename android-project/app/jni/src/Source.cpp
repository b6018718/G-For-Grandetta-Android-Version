...
void MainMenu(Screen screen, Music& music, Fonts& fonts, SDL_Joystick* gGameController)
{
    ...
    bool press = false;
    vector <int> buttonX = { (int)((float)(480.0 - 6.0 / 2.0) / 960.0 * SCREEN_WIDTH), (int)((float)(480.0 - 9.0 / 2.0) / 960.0 * SCREEN_WIDTH), (int)((float)(480.0 - 9.0 / 2.0) / 960.0 * SCREEN_WIDTH) }; //360, 288, 288 : (480 - ((48 * (no of letters + 1)) / 2)
    vector <int> buttonY = { (int) (0.078125 * SCREEN_HEIGHT), (int) (0.234375 * SCREEN_HEIGHT), (int) (0.390625 * SCREEN_HEIGHT) };    //50, 150, 250
    vector <int> buttonWidth = { (int) (0.25 * SCREEN_WIDTH), (int) (0.4 * SCREEN_WIDTH), (int) (0.4 * SCREEN_WIDTH) }; // 240, 384, 384 : (48 * number of letters + 1)
    vector <int> buttonHeight = { (int) (0.075 * SCREEN_HEIGHT), (int) (0.075 * SCREEN_HEIGHT), (int) (0.075 * SCREEN_HEIGHT) }; //48
    int settingsOrigin;
    bool quit = false;
    bool stop = false;
    int background;
    ...
    float fingerX = 0;
    float fingerY = 0;
    float arrowWidth = (int) (SCREEN_WIDTH * 0.075); //48
    float arrowX = buttonX[0] - (arrowWidth / 2) - (float) (0.034375 * SCREEN_WIDTH); // - 22
	...
	*/
    ...
    while (!quit)
    {
        while (SDL_PollEvent(&event))
        {
            ...
            if (event.type == SDL_JOYAXISMOTION)
            {
                ...
                if (event.jaxis.axis == 1)
                {
                    ...
                    if (event.jaxis.value < -JOYSTICK_DEAD_ZONE)
                    {
                        ...
                        {
                            ++cursorPos;
                            arrowX = (float) (buttonX[cursorPos] - (arrowWidth / 2) - (0.0229166667 * SCREEN_WIDTH));
            ...
            if (event.type == SDL_FINGERUP) //Finger Released from touchscreen
            {
                fingerX = event.tfinger.x * SCREEN_WIDTH;
                fingerY = event.tfinger.y * SCREEN_HEIGHT;
                for (int i = 0; i < buttonX.size(); i++)	//If mouse left clicks the button
                {
                    if (fingerX > buttonX[i] && fingerX < buttonX[i] + buttonWidth[i])
                    {
                        if (fingerY > buttonY[i] && fingerY < buttonY[i] + buttonHeight[i])
                        {
                            switch (i)
                            {
                                case 0:
...
void menuAnimation(Screen screen, int frames, float& backgroundX, Fonts fonts, vector <int> buttonY, int arrowX, int arrowY)
{
    ...
    clear(screen.gScreenSurface);
    backgroundX = --backgroundX; //Move background across
    if (backgroundX == -(SCREEN_WIDTH)) //960 resolution
        backgroundX = 0;
    SDL_Rect origin;
    origin.x = (int) -backgroundX;
    origin.y = 0;
    origin.w = 960;
    origin.h = 640;
    SDL_Rect dest;
    ...
    dest.w = SCREEN_WIDTH;
    SDL_BlitScaled(screen.gPlaySurface, &origin, screen.gScreenSurface, &dest);

    //					Text	X			Y			Font
    screen.displayText("Start", 480, buttonY[0], fonts.font48);
    screen.displayText("Settings", 480, buttonY[1], fonts.font48);
    screen.displayText("Controls", 480, buttonY[2], fonts.font48);
    //screen.displayText(">", arrowX, arrowY + (float) (0.00625 * SCREEN_WIDTH), fonts.font48); // arrowY + 6 COMMENTED OUT FOR MOBILE VERSION
...
bool play(Screen screen, Music music, Fonts fonts)
{
    ...
    int battleBuffer = 0; //Increments with every step
    ...
    screen.messageBox("ADVENTURE TIP: Remember to touch the", "backpack if you ever get lost.", fonts.font24);
...
bool classSelect(Screen screen, Music music, Fonts fonts, Player& player)
{
    vector <int> ClassbuttonX = { (int) ((20.0/960.0) * SCREEN_WIDTH), (int) ((326.0/960.0) * SCREEN_WIDTH), (int) ((632.0/960.0) * SCREEN_WIDTH) };
    vector <int> ClassbuttonY = { (int) ((120.0/640.0) * SCREEN_HEIGHT), (int) ((120.0/640.0) * SCREEN_HEIGHT), (int) ((120.0/640.0) * SCREEN_HEIGHT) };
    int Classbuttonwidth = (int) ((300.0/960.0) * SCREEN_WIDTH);
    int Classbuttonheight = (int) ((480.0/640.0) * SCREEN_HEIGHT);
    ...
    int ClassBoxWidth = Classbuttonwidth;
    int ClassBoxSize = ClassBoxWidth;
    int ClassBoxHeight = Classbuttonheight;
    SDL_Rect dest;
    ...
    while (!quit)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (event.type == SDL_FINGERUP)
            {
                ...
                for (int i = 0; i < ClassbuttonX.size(); i++)
                {
                    if (fingerX > ClassbuttonX[i] && fingerX < ClassbuttonX[i] + ClassBoxWidth)
                    {
                        if (fingerY > ClassbuttonY[i] && fingerY < ClassbuttonY[i] + ClassBoxHeight)
                        {
                            classSelected = i;
                        }
                    }
        ...
        //dest.h = SCREEN_HEIGHT;   //TO DO: Make boxes right size for touch down event
        //dest.w = SCREEN_WIDTH;
        SDL_Rect screenDest;
        screenDest.h = SCREEN_HEIGHT;
        screenDest.w = SCREEN_WIDTH;

        SDL_BlitScaled(screen.gPlaySurface, NULL, screen.gScreenSurface, &screenDest);
        SDL_BlitScaled(screen.gText, NULL, screen.gScreenSurface, &dest);
        SDL_UpdateWindowSurface(screen.gWindow);
...
void updateSprite(Screen screen, Player& player)
{
    ...
    SDL_Rect mapLoc;
    mapLoc.x = (int) (player.x / 960.0 * SCREEN_WIDTH);
    mapLoc.y = (int) (player.y / 640.0 * SCREEN_HEIGHT);
    mapLoc.h = (int) (player.spriteSizeX /960.0 * SCREEN_WIDTH);
    mapLoc.w = (int) (player.spriteSizeY / 640.0 * SCREEN_HEIGHT);
    ...
    if (!player.dir.left && !player.dir.right && !player.dir.up && !player.dir.down)
    {
        ...
        if (player.facing.left)
        {
            ...
            sheet.y = 0 * 32;
            SDL_BlitScaled(screen.gSprite, &sheet, screen.gScreenSurface, &mapLoc);
        }
...
bool settings(Screen screen, Music& music, Fonts fonts)
{
    ...
    dest.w = SCREEN_WIDTH;
    dest.h = SCREEN_HEIGHT;
    SDL_BlitScaled(screen.gPlaySurface, NULL, screen.gScreenSurface, &dest);
    ...
    vector <int> VolbuttonX = { (int) ((300.0/960.0) * SCREEN_WIDTH), (int) ((500.0/960.0) * SCREEN_WIDTH) };
    vector <int> VolbuttonY = { (int) ((200.0/640.0) * SCREEN_HEIGHT), (int) ((200.0/640.0) * SCREEN_HEIGHT) };
    int VolbuttonWidth = (int) ((64.0/960.0) * SCREEN_WIDTH);
    int VolbuttonHeight = (int) ((64.0/640.0) * SCREEN_HEIGHT);
    int mouseX = 0;
    int mouseY = 0;
    int fingerX = 0;
    int fingerY = 0;
    int settingsBoxX = 0;
    int settingsBoxY = 0;
    int settingsBoxWidth = (64/960) * SCREEN_WIDTH;
    int settingsBoxHeight = (32/640) * SCREEN_HEIGHT;
    float tempVol;
    ...
    while (!quit)
    {
        while (SDL_PollEvent(&event))
        {
            ...
            {
                fingerX = (int) (event.tfinger.x * SCREEN_WIDTH);
                fingerY = (int) (event.tfinger.y * SCREEN_HEIGHT);

                for (size_t i = 0; i < VolbuttonX.size(); i++)
                {
                    if (fingerX > VolbuttonX[i] && fingerX < VolbuttonX[i] + VolbuttonWidth)
                    {
                        if (fingerY > VolbuttonY[i] && fingerY < VolbuttonX[i] + VolbuttonWidth)
                        {
                            settingsBoxX = VolbuttonX[i];
                            settingsBoxY = VolbuttonY[i];
                            settingsBoxVisible = true;
            ...
            if(event.type == SDL_FINGERUP)
            {
                ...
                for (int i = 0; i < VolbuttonX.size(); i++)
                {
                    if (fingerX > VolbuttonX[i] && fingerX < VolbuttonX[i] + VolbuttonWidth)
                    {
                        if (fingerY > VolbuttonY[i] && fingerY < VolbuttonY[i] + VolbuttonHeight)
                        {
                            switch (i)
                            {
                                case 0:
...
void DrawEXPBar(int posX, int posY, double currentStat, double maxStat, string colour, Screen& screen)
{
    ...
    SDL_Rect topRectangle = { posX + (4 * 960), posY, 520, 4 };     			 // {X, Y, H, W}
    ...
    Uint8 white = (Uint8) 0xFFF;

    SDL_FillRect(screen.gBattleTextBox, &topRectangle, SDL_MapRGB(screen.gBattleTextBox->format, white, white, white));
    SDL_FillRect(screen.gBattleTextBox, &botRectangle, SDL_MapRGB(screen.gBattleTextBox->format, white, white, white));
    SDL_FillRect(screen.gBattleTextBox, &leftRectangle, SDL_MapRGB(screen.gBattleTextBox->format, white, white, white));
    SDL_FillRect(screen.gBattleTextBox, &rightRectangle, SDL_MapRGB(screen.gBattleTextBox->format, white, white, white));
...
bool turnBasedBattle(Player& player, Screen& screen, Maps& maps, Fonts fonts, Music& music, Mobs mobs)
{
    ...
    if (!screen.loadMedia(screen.gBattleBg, selectBackground(player.currentMap)))
        SDL_Log("Failed to load map media");
    if (!screen.loadMedia(screen.gEnemy, enemy.imgSrc))
        SDL_Log("Failed to enemy media");
...
bool openInventory(Screen& screen, Player& player, Music& music, Fonts& fonts)
{
    ...
    //screen.loadMedia(screen.gWeaponIcon, "images/coin.png");
...
void setScreenSize()
{
    ...
    if (screenEvent.window.data1 < screenEvent.window.data2)
    {
        SCREEN_WIDTH = screenEvent.window.data1;
        SCREEN_HEIGHT = screenEvent.window.data2;
    }
    else
    {
        SCREEN_WIDTH = screenEvent.window.data1;
...
bool playCredits(Screen& screen, Player& player, Music& music, Fonts& fonts) {
    ...
    SDL_Event event;
    SDL_FillRect(screen.gScreenSurface, &creditsBackground,
                 SDL_MapRGB(screen.gScreenSurface->format, 0x000, 0x000, 0x000));
    int creditsPos = 0;
    if (player.badEnd == true) {
        music.PlayBadEnd();
        gameExit = screen.messageBox("The battle is over", "You have defeated the demon Lord.",
                                     fonts.font24);
        gameExit = screen.messageBox("After the battle " + player.name + " is", "approached by...",
                                     fonts.font24);
        gameExit = screen.messageBox("No one.", "", fonts.font24);
        gameExit = screen.messageBox("The village remains empty.",
                                     "You are it's last and only resident.", fonts.font24);
        ...
        while (creditsPos < 9500 && gameExit == false) {
            ...
            screen.displayText("G For Grandetta", SCREEN_WIDTH / 2, 1000 + (160 * 1) - creditsPos,
                               fonts.font48);
            screen.displayText("Lead Programmer", SCREEN_WIDTH / 2, 1000 + (160 * 3) - creditsPos,
                               fonts.font48);
            screen.displayText("Ethan Cumberland", SCREEN_WIDTH / 2, 1000 + (160 * 4) - creditsPos,
                               fonts.font48);
            screen.displayText("Battle Programmer", SCREEN_WIDTH / 2, 1000 + (160 * 7) - creditsPos,
                               fonts.font48);
            screen.displayText("Steven Edeson", SCREEN_WIDTH / 2, 1000 + (160 * 8) - creditsPos,
                               fonts.font48);
            screen.displayText("UI Design", SCREEN_WIDTH / 2, 1000 + (160 * 11) - creditsPos,
                               fonts.font48);
            screen.displayText("Ethan Collins", SCREEN_WIDTH / 2, 1000 + (160 * 12) - creditsPos,
                               fonts.font48);
            screen.displayText("Lead Artist", SCREEN_WIDTH / 2, 1000 + (160 * 15) - creditsPos,
                               fonts.font48);
            screen.displayText("Jack Corton", SCREEN_WIDTH / 2, 1000 + (160 * 16) - creditsPos,
                               fonts.font48);
            screen.displayText("Map Design", SCREEN_WIDTH / 2, 1000 + (160 * 19) - creditsPos,
                               fonts.font48);
            screen.displayText("Anthony Dranfield", SCREEN_WIDTH / 2,
                               1000 + (160 * 20) - creditsPos, fonts.font48);
            screen.displayText("Music Provided by Eric Skiff", SCREEN_WIDTH / 2,
                               1000 + (160 * 22) - creditsPos, fonts.font24);
            screen.displayText("Available at http://ericskiff.com/music/", SCREEN_WIDTH / 2,
                               1000 + (160 * 22.5) - creditsPos, fonts.font24);
            screen.displayText("Special Thanks To", SCREEN_WIDTH / 2,
                               1000 + (160 * 25) - creditsPos, fonts.font48);
            screen.displayText("Medhi Mir-Ghasemi", SCREEN_WIDTH / 2,
                               1000 + (160 * 26) - creditsPos, fonts.font48);
            screen.displayText("G For Grandetta", SCREEN_WIDTH / 2, 1000 + (160 * 28) - creditsPos,
                               fonts.font48);
            screen.displayText("Enhanced Edition", SCREEN_WIDTH / 2,
                               1000 + (160 * 28.5) - creditsPos, fonts.font48);
            screen.displayText("Lead Programmer", SCREEN_WIDTH / 2, 1000 + (160 * 30) - creditsPos,
                               fonts.font48);
            screen.displayText("Anthony Dranfield", SCREEN_WIDTH / 2,
                               1000 + (160 * 31) - creditsPos, fonts.font48);
            screen.displayText("SDL 2.0 Library", SCREEN_WIDTH / 2, 1000 + (160 * 33) - creditsPos,
                               fonts.font48);
            screen.displayText("Sam Lantinga", SCREEN_WIDTH / 2, 1000 + (160 * 34) - creditsPos,
                               fonts.font48);
            screen.displayText("SDL Mixer Library", SCREEN_WIDTH / 2,
                               1000 + (160 * 36) - creditsPos, fonts.font48);
            screen.displayText("Sam Lantinga", SCREEN_WIDTH / 2, 1000 + (160 * 37) - creditsPos,
                               fonts.font48);
            screen.displayText("Stephane Peter", SCREEN_WIDTH / 2, 1000 + (160 * 38) - creditsPos,
                               fonts.font48);
            screen.displayText("Ryan Gordon", SCREEN_WIDTH / 2, 1000 + (160 * 39) - creditsPos,
                               fonts.font48);
            screen.displayText("SDL TTF Library", SCREEN_WIDTH / 2, 1000 + (160 * 41) - creditsPos,
                               fonts.font48);
            screen.displayText("Sam Lantinga", SCREEN_WIDTH / 2, 1000 + (160 * 42) - creditsPos,
                               fonts.font48);
            screen.displayText("Player Kills: " + to_string(player.killCount), SCREEN_WIDTH / 2,
                               1000 + (160 * 44) - creditsPos, fonts.font24);
            screen.displayText("Player Deaths: " + to_string(player.deathCount), SCREEN_WIDTH / 2,
                               1000 + (160 * 46) - creditsPos, fonts.font24);
            if (player.deathCount == 0)
                screen.displayText("K/D Ratio: " + to_string(player.killCount), SCREEN_WIDTH / 2,
                                   1000 + (160 * 48) - creditsPos, fonts.font24);
            else
                screen.displayText("K/D Ratio: " + to_string(player.killCount / player.deathCount),
                                   SCREEN_WIDTH / 2, 1000 + (160 * 48) - creditsPos, fonts.font24);
            screen.displayText("Thank you for playing!", SCREEN_WIDTH / 2,
                               1000 + (160 * 50) - creditsPos, fonts.font24);
            creditsPos++;
        ...
        gameExit = screen.messageBox("Thank you brave hero for sparing us.",
                                     "If it wouldn't be too much trouble", fonts.font24);
        gameExit = screen.messageBox("could we move into this village with", "you?", fonts.font24);
        gameExit = screen.messageBox("We know that we can never replace", "your friends.",
                                     fonts.font24);
        gameExit = screen.messageBox("But we would hate to see you ", "so lonely.", fonts.font24);
        gameExit = screen.messageBox("You agree to let the younglings",
                                     "live alongside you in the village.", fonts.font24);
        gameExit = screen.messageBox("The goblins bring about a new",
                                     "age of prosperity and technical", fonts.font24);
        ...
        gameExit = screen.messageBox("...", "", fonts.font24);
        gameExit = screen.messageBox("Hip Hip hurray!!!", "", fonts.font24);
        gameExit = screen.messageBox("You have achieved the GOOD END", "Congratulations!",
                                     fonts.font24);
